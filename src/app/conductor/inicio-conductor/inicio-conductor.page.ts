...
export class InicioConductorPage implements OnInit {
  ...
  getInfo(){    
    this.db.collection('conductor').doc(this.uid).get().forEach(doc=>{
      this.dataService.setDataConductor(doc.data())
      this.db.collection('persona').doc(this.dataService.getDataConductor().id_persona).get().forEach(doc=>{
        this.dataService.setDataConductorPersona(doc.data())
        this.db.collection('furgon').doc(this.dataService.getDataConductor().id_furgon).get().forEach(doc=>{
          ...
          for (let i = 0; i < this.id_auxiliares.length; i++) {
            this.db.collection('auxiliar').doc(this.id_auxiliares[i]).get().forEach(doc=>{
              this.id_auxiliares_p[i] = doc.get('id_persona')
            this.db.collection('persona').doc(this.id_auxiliares_p[i]).get().forEach(doc=>{
          ...
          for (let i = 0; i < this.id_alumnos.length; i++) {
            this.db.collection('alumno').doc(this.id_alumnos[i]).get().forEach(doc=>{
              this.id_alumnos_p[i] = doc.get('id_persona')
            this.db.collection('persona').doc(this.id_alumnos_p[i]).get().forEach(doc=>{
    ...
    if(this.al_ids.length>0 ){
      if(this.bind){
        for (let i = 0; i < this.alums.length; i++) {
            this.db.collection('auxiliar').doc(this.bind).get().forEach(doc=>{
