...
export class RastreoApoderadoPage implements OnInit,OnDestroy {
  ...
  ionViewDidEnter(){
      this.sub = interval(6000).subscribe((func =>{        
        ...
        this.db.collection('alumno').doc(this.dataService.getDataAlumno().id_alumno).get().forEach(doc=>{
      ...
      this.db.collection('furgon').doc(this.dataService.getDataAlumno().id_furgon).get().forEach(doc => {
        ...
        this.db.collection('conductor').doc(this.dataService.getdataFurgon().id_conductor).get().forEach(doc=>{
        ...
        for (let i = 0;  i< this.id_aux.length; i++) {
            this.db.collection('auxiliar').doc(this.id_aux[i]).get().forEach(doc =>{
              if (doc.get('aux_estado')=='1') {
                this.dataService.setDataAuxiliar(doc.data())
                this.db.collection('persona').doc(this.dataService.getDataAuxiliar().id_persona).get().forEach(doc=>{
