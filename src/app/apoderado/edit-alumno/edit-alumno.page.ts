...
export class EditAlumnoPage implements OnInit {
  ...
  constructor(public dataService: DatosService,
    ...
    const alert = await this.alertController.create({
      ...
      buttons:[
        ...
        },
        {
          text: 'Confirmar',
          handler: b =>{
            this.db.collection('alumno').doc(this.dataAlumno.id_alumno).update({
              alu_comentario : this.comentario
            }).catch(err=>console.log(err))
            this.db.collection('persona').doc(this.dataAlumno.id_persona).update({
